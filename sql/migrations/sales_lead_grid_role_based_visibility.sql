-- =============================================
-- Migration: Role-Based Lead Visibility
-- Area Manager: own leads + team leads
-- Sales Representative: own leads + activity-linked leads
-- =============================================
...
BEGIN
  -- ORDER BY
  IF lower(p_order_by) = 'id' THEN v_order_by := 'sl.id';
  ELSIF lower(p_order_by) = 'date_created' THEN v_order_by := 'sl.date_created';
  ELSIF lower(p_order_by) = 'date_updated' THEN v_order_by := 'sl.date_updated';
  ELSE v_order_by := 'sl.id';
  END IF;

  IF upper(p_order_direction) = 'ASC' THEN v_order_direction := 'ASC';
  ELSE v_order_direction := 'DESC';
  END IF;
  ...
  ELSIF v_user_role IN ('Area Manager', 'Sales Manager') THEN
    -- Own leads + all leads created by team members (recursive)
    v_where_clause := 'WHERE sl.isactive = true AND (
      sl.user_created = ' || p_current_user_id || '
      OR sl.assigned_to = ' || p_current_user_id || '
      OR sl.user_created IN (
        SELECT t.userid FROM public.get_salesmanager_child_userids(' || p_current_user_id || ') t
      )
    )';

  ELSIF v_user_role IN ('Territory Manager', 'Field Service Technician') THEN
    v_where_clause := 'WHERE sl.isactive = true AND (
      sl.user_created IN (SELECT t.userid FROM public.get_salesmanager_child_userids(' || p_current_user_id || ') t)
      OR sl.assigned_to = ' || p_current_user_id || '
    )';

  ELSIF v_user_role IN ('Sales Representative', 'Sales Rep') THEN
    -- Own leads + leads linked to activities assigned to them
    v_where_clause := 'WHERE sl.isactive = true AND (
      sl.user_created = ' || p_current_user_id || '
      OR sl.assigned_to = ' || p_current_user_id || '
      OR sl.id IN (
        SELECT CAST(stage_item_id AS INTEGER)
        FROM public.sales_activity_tasks
        WHERE assignedtouserid = ' || p_current_user_id || '
          AND stage_item_id IS NOT NULL
          AND stage_item_id ~ ''^[0-9]+$''
        UNION
        SELECT CAST(stage_item_id AS INTEGER)
        FROM public.sales_activity_calls
        WHERE assignedtouserid = ' || p_current_user_id || '
          AND stage_item_id IS NOT NULL
          AND stage_item_id ~ ''^[0-9]+$''
        UNION
        SELECT CAST(stage_item_id AS INTEGER)
        FROM public.sales_activity_meetings
        WHERE assignedtouserid = ' || p_current_user_id || '
          AND stage_item_id IS NOT NULL
          AND stage_item_id ~ ''^[0-9]+$''
        UNION
        SELECT CAST(stage_item_id AS INTEGER)
        FROM public.sales_activity_events
        WHERE assignedtouserid = ' || p_current_user_id || '
          AND stage_item_id IS NOT NULL
          AND stage_item_id ~ ''^[0-9]+$''
      )
  ...
  IF p_search_text IS NOT NULL AND p_search_text != '' AND p_search_text != 'string' THEN
    v_where_clause := v_where_clause || ' AND (
      ...
      LOWER(sl.lead_type)     LIKE ''%'' || LOWER($2) || ''%''
    )';
  END IF;
